...
class uvm_reg_fifo: uvm_reg
{
  ...
  private uvm_reg_field value;
  private int m_set_cnt;
  private int unsigned m_size;
  ...
  void build() {
    synchronized(this) {
      ...
      value.configure(this, get_n_bits(), 0, "RW", 0, 0, 1, 0, 1);
    }
  }
  ...
  void update(out uvm_status_e      status,
    ...
    for (int i=fifo.length-m_set_cnt; m_set_cnt > 0; i++, m_set_cnt--) {
      if (i >= 0) {
  ...
  //
  uvm_reg_data_t get(string fname="", int lineno=0) {
    synchronized(this) {
  ...
  void do_predict(uvm_reg_item      rw,
	...
	UVM_PREDICT_DIRECT:
	{
	  if (fifo.length != m_size && !m_update_in_progress) {
	    fifo.push_back(this.value.value);
	  }
	}

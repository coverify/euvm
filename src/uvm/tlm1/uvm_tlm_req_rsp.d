...
module uvm.tlm1.uvm_tlm_req_rsp;

class uvm_tlm_req_rsp_channel(REQ=int, RSP=REQ): uvm_component
{
  alias uvm_tlm_req_rsp_channel!(REQ, RSP) this_type;

  enum string type_name = "uvm_tlm_req_rsp_channel!(REQ,RSP)";
  ...
  uvm_master_imp!(REQ, RSP, this_type,
		  uvm_tlm_fifo!REQ, uvm_tlm_fifo!RSP) master_export;
  ...
  uvm_slave_imp!(REQ, RSP, this_type,
		 uvm_tlm_fifo!REQ, uvm_tlm_fifo!RSP) slave_export;

  // port aliases for backward compatibility
  uvm_put_export!REQ blocking_put_request_export,
    nonblocking_put_request_export;

  uvm_get_peek_export!REQ get_request_export, blocking_get_request_export,
    nonblocking_get_request_export, peek_request_export,
    blocking_peek_request_export, nonblocking_peek_request_export,
    blocking_get_peek_request_export, nonblocking_get_peek_request_export;

  uvm_put_export!REQ blocking_put_response_export,
    nonblocking_put_response_export;

  uvm_get_peek_export!RSP get_response_export, blocking_get_response_export,
    nonblocking_get_response_export, peek_response_export,
    blocking_peek_response_export, nonblocking_peek_response_export,
  ...
  uvm_master_imp!(REQ, RSP, this_type, uvm_tlm_fifo!(REQ), uvm_tlm_fifo!(RSP))
    blocking_master_export, nonblocking_master_export;

  uvm_slave_imp!(REQ, RSP, this_type, uvm_tlm_fifo!(REQ), uvm_tlm_fifo!(RSP))
    blocking_slave_export, nonblocking_slave_export;
  ...
  public this(string name=null, uvm_component parent=null,
    ...
    synchronized(this) {
      ...
      m_request_fifo  = new uvm_tlm_fifo!(REQ)
	("request_fifo",  this, request_fifo_size);

      m_response_fifo = new uvm_tlm_fifo!(RSP)
	("response_fifo", this, response_fifo_size);

      request_ap      = new uvm_analysis_port!(REQ)
	("request_ap",  this);

      response_ap     = new uvm_analysis_port!(RSP)
	("response_ap", this);

      put_request_export       = new uvm_put_export!(REQ)
	("put_request_export", this);

      get_peek_request_export  = new uvm_get_peek_export!(REQ)
	("get_peek_request_export",  this);
	...
	("get_peek_response_export", this);
      ...
      master_export   = new uvm_master_imp!(REQ, RSP, this_type,
					    uvm_tlm_fifo!REQ, uvm_tlm_fifo!RSP)
	("master_export", this, m_request_fifo, m_response_fifo);

      slave_export    = new uvm_slave_imp!(REQ, RSP, this_type,
					   uvm_tlm_fifo!REQ, uvm_tlm_fifo!RSP)
	("slave_export",  this, m_request_fifo, m_response_fifo);
  ...
  public void create_aliased_exports() {
    synchronized(this) {
      // request
  ...
  uvm_tlm_req_rsp_channel!(REQ, RSP)
{
  ...
  alias uvm_tlm_transport_channel!(REQ, RSP) this_type;
  ...
  public this (string name=null, uvm_component parent=null) {
    synchronized(this) {
      super(name, parent, 1, 1);
      transport_export = new uvm_transport_imp!(REQ, RSP, this_type)
	 ("transport_export", this);
    }
  }

  // task
  public void transport (REQ request, out RSP response) {
    uvm_tlm_fifo!(REQ) req_fifo;
    uvm_tlm_fifo!(RSP) rsp_fifo;
    synchronized(this) {
      req_fifo = this.m_request_fifo;
      rsp_fifo = this.m_response_fifo;
    }
    req_fifo.put( request );
    rsp_fifo.get( response );
  }

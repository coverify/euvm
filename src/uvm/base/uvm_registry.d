...
class uvm_component_registry(T=uvm_component, string Tname="<unknown>"):
  uvm_object_wrapper if(is(T: uvm_component))
  {
    ...
    override public uvm_component create_component (string name,
	...
	if(_me is null) {
	  ...
	  if (top !in _me_pool) {
	    ...
	    _me_pool[top] = new this_type;
	    f.register(_me_pool[top]);
	  }
	  _me = _me_pool[top];
	}
	return _me;
    ...
    static public T create(string name = "", uvm_component parent = null,
			   string contxt = "") {
      // synchronized {
      uvm_object obj;
      uvm_factory f = uvm_factory.get();
	...
	contxt = parent.get_full_name();
      ...
      T retval = cast(T) obj;
      if (retval is null) {
	string msg = "Factory did not return a component of type '" ~ type_name ~
    ...
    static public void set_type_override (uvm_object_wrapper override_type,
					  bool replace=1) {
    ...
    static public void set_inst_override(uvm_object_wrapper override_type,
					 string inst_path,
	...
	if (inst_path == "") {
      ...
      uvm_factory.get.set_inst_override_by_type(get(), override_type, inst_path);
    }
...
class uvm_object_registry (T = uvm_object, string Tname = "<unknown>"):
  uvm_object_wrapper
{
  alias uvm_object_registry!(T, Tname) this_type;
  ...
  override public uvm_object create_object(string name="") {
    T obj;
